...
const secretFields = new Set(["s2a.adminApiKey"])
...
export function validateConfig(config) {
  ...
  for (const rule of config.alerts.accounts) {
    const [provider, accountId] = splitAccount(rule.account)
    if (provider !== "openai" || !isPositiveInteger(accountId)) {
      throw new Error("告警账号必须选择有效的 S2A OpenAI OAuth 账号")
    }
    if (
  ...
  if (config.alerts.enabled) {
    if (!config.alerts.accounts.length && !config.alerts.sla.enabled) {
      throw new Error("启用告警前至少配置一个额度账号或启用 SLA 监控")
    }
...
function normalizeAlertAccounts(value) {
  ...
  return value
    .map(rule => {
      const [provider, accountId] = splitAccount(String(rule?.account ?? "").trim())
      return {
        account:
          provider.toLowerCase() === "openai" && isPositiveInteger(accountId)
            ? `openai:${Number(accountId)}`
            : "",
