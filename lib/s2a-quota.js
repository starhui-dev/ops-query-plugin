...
export async function listS2aOAuthAccounts(config) {
  ...
  do {
    ...
    const data = readResponseData(await s2aRequest(config, `/api/v1/admin/accounts?${query}`))
...
export async function listS2aQuotaAccountOptions(config) {
  return buildS2aQuotaAccountOptions(await listS2aOAuthAccounts(config))
}
...
export async function queryS2aQuota(config, timeZone = "Asia/Shanghai", accountIds = []) {
  const accounts = selectAccounts(await listS2aOAuthAccounts(config), accountIds)
  const results = []

  for (const account of accounts) {
    try {
      const quota = readResponseData(
        await s2aRequest(config, `/api/v1/admin/openai/accounts/${account.id}/quota`),
      )
...
export async function queryS2aQuotaUsage(config, accountIds = []) {
  const accounts = selectAccounts(await listS2aOAuthAccounts(config), accountIds)
  if (!accounts.length) return []

  const payload = await s2aRequest(config, "/api/v1/admin/accounts/usage/batch", {
    method: "POST",
...
async function s2aRequest(config, path, options = {}) {
  return requestJson(
