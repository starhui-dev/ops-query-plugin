...
const editableFields = [
  ...
  "access.queryUsers",
  "balanceRequests.enabled",
  "balanceRequests.maxAmount",
  "balanceRequests.adminUsers",
  "balanceRequests.emailVerification.enabled",
  "balanceRequests.emailVerification.smtpHost",
  "balanceRequests.emailVerification.smtpPort",
  "balanceRequests.emailVerification.smtpSecure",
  "balanceRequests.emailVerification.smtpUser",
  "balanceRequests.emailVerification.smtpPassword",
  "balanceRequests.emailVerification.from",
  "alerts.enabled",
...
function normalizeConfig(config) {
  return {
    ...
    balanceRequests: {
      enabled: config.balanceRequests?.enabled !== false,
      maxAmount: normalizeBalanceMaxAmount(config.balanceRequests?.maxAmount),
      adminUsers: normalizeIdList(config.balanceRequests?.adminUsers),
      emailVerification: normalizeEmailVerification(config.balanceRequests?.emailVerification),
    },
    alerts: {
...
export function getGuobaConfig() {
  ...
  config.balanceRequests.emailVerification.smtpPassword = ""
  return config
}
...
export function validateConfig(config) {
  ...
  const balanceMaxAmount = Number(config.balanceRequests?.maxAmount ?? 100)
  if (!Number.isFinite(balanceMaxAmount) || balanceMaxAmount <= 0 || balanceMaxAmount > 1_000_000) {
    throw new Error("余额申请单笔上限必须大于 0 且不超过 1000000")
  }
  validateEmailVerification(config.balanceRequests?.emailVerification)
  if (config.alerts.enabled) {
    if (!config.alerts.accounts.length && !config.alerts.sla.enabled) {
      throw new Error("启用告警前至少配置一个额度账号或 SLA 监控")
    }
...
function normalizeBalanceMaxAmount(value) {
  const amount = Number(value)
  return Number.isFinite(amount) && amount > 0 && amount <= 1_000_000 ? amount : 100
}
...
function normalizeEmailVerification(value) {
  ...
  return {
    ...
    smtpPort: Number.isInteger(port) && port >= 1 && port <= 65535 ? port : 465,
...
function validateEmailVerification(config) {
  if (!config?.enabled) return
  if (!config.smtpHost) throw new Error("启用邮箱验证码前必须填写 SMTP 地址")
  if (!config.from || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(config.from)) {
    throw new Error("启用邮箱验证码前必须填写有效的发件人邮箱")
  }
  if (!Number.isInteger(config.smtpPort) || config.smtpPort < 1 || config.smtpPort > 65535) {
    throw new Error("SMTP 端口必须是 1 至 65535 之间的整数")
  }
  if (config.smtpUser && !config.smtpPassword) {
    throw new Error("SMTP 用户已填写，但 SMTP 密码为空")
  }
}
...
function mergeConfig(defaults, current) {
  return {
    ...
    balanceRequests: { ...defaults.balanceRequests, ...current.balanceRequests },
