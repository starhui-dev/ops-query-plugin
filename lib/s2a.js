...
export async function queryS2aMonitors(config) {
  ...
  return Promise.all(monitors.map(monitor => appendLatestPing(config, monitor)))
}
...
async function appendLatestPing(config, monitor) {
...
async function s2aRequest(config, path) {
  return requestJson(
