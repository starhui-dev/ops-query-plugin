...
export async function queryS2aV2Monitor(config, fetchImpl) {
  ...
  const [snapshot, matrix, models] = await Promise.all([
    s2aV2Request(config, `${basePath}/snapshot?${query}`, fetchImpl),
    s2aV2Request(config, `${basePath}/matrix?${matrixQuery}`, fetchImpl),
    s2aV2Request(config, `${basePath}/models?${query}`, fetchImpl),
  ])
...
async function s2aV2Request(config, path, fetchImpl) {
  return requestJson(
