...
export const MAX_BALANCE_REQUEST_ELIGIBLE_BALANCE = 5
...
export function assertBalanceRequestEligible(user) {
  const balance = Number(user?.balance)
  if (!Number.isFinite(balance)) {
    throw new Error("无法读取 S2A 当前余额，暂不能申请余额")
  }
  if (balance > MAX_BALANCE_REQUEST_ELIGIBLE_BALANCE) {
    throw new Error(`当前 S2A 余额为 ${balance}，大于 5，不允许申请余额`)
  }
  return balance
}
