...
余额申请使用 Sub2API 用户邮箱查找对应账号，群成员先发送 `#绑定账号 user@example.com`，
插件通过 SMTP 向该邮箱发送 6 位验证码；成员再发送 `#验证码 123456`，验证通过后生成绑定申请，
管理员回复申请消息 `#通过` 后绑定才生效。之后发送 `#申请余额 10` 或 `#申请余额 10 续费`，
管理员回复申请消息 `#通过` 会调用 Sub2API 管理接口增加余额。绑定和申请状态保存在插件目录的
