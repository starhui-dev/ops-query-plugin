import { CPA_QUOTA_PLATFORM_KEYS, normalizeCpaProvider } from "./quota-platforms.js"
...
export function parseQuotaAccountReference(value) {
  ...
  if (platform.toLowerCase() === "grok") {
    return normalizeQuotaAccount("cpa", "xai", accountId)
  }
  if (platform && isPositiveInteger(accountId)) {
...
export function normalizeQuotaAccount(sourceValue, platformValue, accountIdValue) {
  ...
  if (source === "s2a") {
    ...
    if (!platform || !isPositiveInteger(accountId)) return null
    return { source, platform, accountId }
  }
  return null
}
