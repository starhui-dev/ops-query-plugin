...
export async function listCpaQuotaAccounts(config, fetchImpl) {
  ...
  return files.map(cpaQuotaAccountOption)
}

export async function listCpaQuotaAccountOptions(config, fetchImpl) {
  const files = getCpaQuotaFiles(await fetchCpaAuthFiles(config, fetchImpl))
  const results = await Promise.all(
    files.map(file => buildCpaQuotaResult(config, file, "Asia/Shanghai", fetchImpl)),
  )
  return results
    .filter(result => result.windows.length)
    .map(result => cpaQuotaAccountOption(result.account))
}

function cpaQuotaAccountOption(account) {
  const platform = normalizeCpaProvider(account?.platform ?? account?.provider ?? account?.type)
  const descriptor = cpaQuotaPlatform({ provider: platform })
  const accountId = account?.id ?? account?.auth_index ?? account?.authIndex
  return {
    label: `CPA · ${descriptor?.label || platform || "未知平台"} · ${account?.name || accountName(account)}`,
...
export async function queryCpaQuota(config, timeZone = "Asia/Shanghai", accounts = [], fetchImpl) {
  ...
  const results = await Promise.all(
    files.map(file => buildCpaQuotaResult(config, file, timeZone, fetchImpl)),
  )
...
async function buildCpaQuotaResult(config, file, timeZone, fetchImpl) {
  ...
  const account = {
    ...
    platform,
    type: String(file?.type ?? "oauth").toLowerCase(),
    source: "cpa",
  }
  const base = {
    source: "cpa",
  ...
  try {
    ...
    if (!quota.windows.length) return { ...base, windows: [] }
    return { ...base, ...quota }
  } catch (error) {
...
function buildXaiWindows(payloads) {
  ...
  for (const payload of payloads) {
    ...
    } else if (isZeroWeeklyUsage(config, periodType, resetAt)) {
      // proto3 省略 0，有周周期但没有百分比时按未使用处理
      addXaiWindow(windows, {
        label: "每周额度",
        usedPercent: 0,
        resetAt,
...
function isZeroWeeklyUsage(config, periodType, resetAt) {
  if (periodType.includes("month")) return false
  if (config.monthlyLimit !== undefined || config.monthly_limit !== undefined) return false
  return Boolean(resetAt || periodType.includes("week"))
}
