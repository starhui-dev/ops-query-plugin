...
export async function fetchLatestCodexRadarImage(proxy = {}, fetchImpl) {
  return withProxy(proxy, requestFetch => fetchLatestImage(requestFetch), fetchImpl)
}

async function fetchLatestImage(fetchImpl) {
...
export function parseCodexRadarImageUrl(html, pageUrl = CODEX_RADAR_URL) {
  ...
  if (!figure || !/(?:雷达|Codex 站)速览图更新时间/.test(figure)) {
