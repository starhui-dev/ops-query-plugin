...
插件通过 SMTP 向该邮箱发送 6 位验证码；成员再发送 `#S2A验证码 123456`，验证通过后直接绑定，
验证码通过后会立即绑定邮箱，可用 `#S2A余额` 查询当前余额。之后发送 `#S2A申请余额 10` 或 `#S2A申请余额 10 续费`，
机器人主人或配置的机器人审批人员回复申请消息 `#S2A通过` 会调用 Sub2API 管理接口增加余额。绑定和申请状态保存在插件目录的
