...
export function formatCpaStatus(report) {
  ...
  for (const file of files) {
    ...
    lines.push(
      [
        `${providerDisplayName(normalizeProvider(file?.provider ?? file?.type))} · ${maskAccount(accountName(file))} · ${credentialStatusLabel(file)}`,
...
function buildCredentialStatusSection(file, timeZone) {
  ...
  return {
    ...
    title: maskAccount(accountName(file)),
    subtitle: "凭据文件已隐藏",
    ...
    rows: [
      ...
      { label: "文件", value: formatBytes(file?.size), detail: "凭据文件已隐藏", tone: "info" },
      {
        label: "更新时间",
...
export function formatCpaResult(result) {
  const name = maskAccount(accountName(result.file))
...
function buildQuotaImageSection(result, timeZone) {
  const title = maskAccount(accountName(result.file))
  if (result.error) {
    return {
      title,
      subtitle: "Codex 账号",
