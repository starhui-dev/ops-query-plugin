...
| 命令                         | 作用                          |
| ---------------------------- | ----------------------------- |
| `#Codex额度` / `#Codex 额度` | 查询 S2A Codex OAuth 账号额度 |
...
Codex 额度和重置命令不区分英文字母大小写，并允许在 `Codex` 与中文命令词之间添加空格。
...
| `alerts.accounts`       | S2A OAuth 账号及其额度告警阈值         |
...
测试覆盖查询权限、配置校验、Codex 额度与告警、重置订阅、SLA 告警，以及 S2A V1
渠道历史与 V2 聚合指标处理。
