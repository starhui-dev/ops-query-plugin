...
- 支持群成员绑定 Sub2API 用户账号并申请余额；绑定和余额申请均落盘记录，群主／管理员可
  直接回复申请消息 `#S2A通过` 或 `#S2A拒绝`，通过后自动调用 Sub2API 管理接口增加余额。
...
| `#S2A绑定 <邮箱>`            | 发送邮箱验证码，验证后提交账号绑定申请 |
| `#S2A验证码 <6位数字>`       | 验证邮箱并生成绑定申请                 |
| `#S2A申请余额 <金额> [备注]` | 提交余额增加申请                       |
| `#S2A通过` / `#S2A拒绝`      | 管理员回复申请消息进行审批             |
| `#运维查询帮助`              | 显示命令帮助                           |
...
余额申请使用 Sub2API 用户邮箱查找对应账号，群成员先发送 `#S2A绑定 user@example.com`，
插件通过 SMTP 向该邮箱发送 6 位验证码；成员再发送 `#S2A验证码 123456`，验证通过后生成绑定申请，
管理员回复申请消息 `#S2A通过` 后绑定才生效。之后发送 `#S2A申请余额 10` 或 `#S2A申请余额 10 续费`，
管理员回复申请消息 `#S2A通过` 会调用 Sub2API 管理接口增加余额。绑定和申请状态保存在插件目录的
`data/balance-requests.json`，验证码只保存哈希并有 10 分钟有效期、5 次错误上限和发送频率限制。
申请前会实时检查 S2A 当前余额，余额大于 5 时不允许申请；余额无法读取时也不会创建申请。
需要在锅巴“邮箱验证码”区域配置 SMTP 地址、端口、账号、密码和发件人；未配置时绑定功能不会发送邮件。
若管理员不直接回复消息，也可以使用 `#S2A通过 BR-...` 或 `#S2A拒绝 BR-...` 指定申请编号。
