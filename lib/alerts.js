...
export function formatQuotaAlerts(alerts) {
  return [
    ...
    ...alerts.map(alert => {
      const name = singleLine(accountName(alert.result.file))
      return [
        `Codex · ${name}`,
        `当前最低  ${formatPercent(alert.remainingPercent)}`,
        `告警阈值  ${formatPercent(alert.thresholdPercent)}`,
        `账号标识  ${singleLine(alert.account.authIndex)}`,
      ].join("\n")
    }),
  ].join("\n\n")
}
...
export function buildQuotaAlertImageData(alerts, timeZone = "Asia/Shanghai", now = Date.now()) {
  return {
    ...
    sections: alerts.map(alert => {
      return {
        title: singleLine(accountName(alert.result.file)),
        subtitle: `Codex · ${singleLine(alert.account.authIndex)}`,
        badge: "额度偏低",
        badgeTone: "danger",
        rows: [
          {
