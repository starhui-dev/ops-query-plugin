...
export const BALANCE_REQUEST_STATE_VERSION = 2
...
export function findBindingByEmail(state, email) {
  const normalized = normalizeEmail(email)
  return (
    Object.values(state.bindings).find(
      binding => String(binding?.email || "").toLowerCase() === normalized,
    ) ?? null
  )
}

export function bindAccount(state, { groupId, userId, accountId, email, approvedBy = "" }) {
  const binding = {
  ...
  if (accountId !== undefined && accountId !== null && accountId !== "") {
    binding.accountId = parseS2aUserId(accountId)
  }
...
function createRequest(state, input) {
  ...
  if (input.accountId !== undefined && input.accountId !== null && input.accountId !== "") {
    request.accountId = parseS2aUserId(input.accountId)
  }
...
function normalizeState(value) {
  ...
  return {
    ...
    verifications:
      source.verifications && typeof source.verifications === "object" ? source.verifications : {},
    verificationSends: Array.isArray(source.verificationSends) ? source.verificationSends : [],
  }
}
